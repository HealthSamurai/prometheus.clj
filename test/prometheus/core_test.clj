...
(defn new-context [x] (atom x))

(defn dispatch [ztx req]
  ((:metrics/dispatch @ztx) req))

(t/deftest test-box-metrics

  (def ctx-c (new-context {:box/metrics (sut/new-registry)}))

  (sut/counter ctx-c :my_counter {:box "aidbox"})

  (Thread/sleep 50)
  (t/is (= (sut/get-metric ctx-c :my_counter {:box "aidbox"}) 1))


  (sut/counter ctx-c :my_counter {:box "aidbox"})

  (Thread/sleep 50)
  (t/is (= (sut/get-metric ctx-c :my_counter {:box "aidbox"}) 2))

  (t/is (= (sut/serialize ctx-c)
           "# TYPE my_counter counter\nmy_counter{box=\"aidbox\",} 2\n"))

  (sut/counter-add ctx-c :my_counter {:box "aidbox"} 5)

  (Thread/sleep 50)
  (t/is (= (sut/get-metric ctx-c :my_counter {:box "aidbox"}) 7))

  (sut/counter ctx-c :minutes :slow_counter {:box "aidbox"})
  (Thread/sleep 50)
  (t/is (= (sut/serialize ctx-c :minutes)
           "# TYPE slow_counter counter\nslow_counter{box=\"aidbox\",} 1\n"))

  ctx-c

  (def ctx-g (new-context {:box/metrics (sut/new-registry)}))
  (sut/gauge ctx-g :my_gauge {:box "aidbox"} 10.34)

  (Thread/sleep 50)
  (t/is (= (sut/get-metric ctx-g :my_gauge {:box "aidbox"}) 10.34))

  (sut/gauge ctx-g :my_gauge {:box "aidbox"} 7.85)

  (Thread/sleep 50)
  (t/is (= (sut/get-metric ctx-g :my_gauge {:box "aidbox"}) 7.85))

  (Thread/sleep 50)
  (t/is (= (sut/serialize ctx-g)
           "# TYPE my_gauge gauge\nmy_gauge{box=\"aidbox\",} 7.85\n"))

  (def ctx-h (new-context {:box/metrics (sut/new-registry)}))
  (sut/register-metric-meta ctx-h :my_histogram :buckets [0 5 30 50 100 200 500 1000 2000 3000 10000 20000])


  (Thread/sleep 100)
  (sut/histogram ctx-h :my_histogram {:label "aidbox"} 35)
  (sut/histogram ctx-h :my_histogram {:label "aidbox"} 234)
  (sut/histogram ctx-h :my_histogram {:label "aidbox"} 230)
  (sut/histogram ctx-h :my_histogram {:label "aidbox"} 290)
  (sut/histogram ctx-h :my_histogram {:label "aidbox"} 12000)
  (sut/histogram ctx-h :my_histogram {:label "aidbox"} 100000)
  (sut/histogram ctx-h :my_histogram {:label "other"} 100000)
  (sut/histogram ctx-h :my_histogram {:label "other"} 12000)

  (Thread/sleep 100)

  (t/is (= (sut/serialize ctx-h)
